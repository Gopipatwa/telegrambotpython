...
def cmd(message):
    for i in datafetch():
        if(i['teleusername'] == message.chat.username):
            ...
            bot.send_message(message.chat.id,text=f"Hello {message.chat.username}", reply_markup=markup)
                ...
                if len(msgsplit[1])!=8:
                    ...
                    # bot.send_message(message.chat.id,text=msgsplit[1])
                                ...
                                if i['teleusername'] == message.chat.username:
                                    try:
                                        try:
                                            ...
                                            print(f'data1/{i["userrole"]}/{date}/{msgsplit[0]}/{i["username"]}-{date}.{msgsplit[0]}')
                                            ...
                                            bot.send_message(message.chat.id,text="Please Contact with support on telegram for assistance @rohanroy123")
                                            print(f'data1/{i["userrole"]}/{max(li)}/{msgsplit[0]}/{i["username"]}-{max(li)}.{msgsplit[0]}')
                                            bot.send_document(message.chat.id, open(f'data1/{i["userrole"]}/{max(li)}/{msgsplit[0]}/{i["username"]}-{max(li)}.{msgsplit[0]}', 'rb'))
                                            # print(a)
                                            # print(len(li))
                        ...
                        for data in datafetch():
                            if message.chat.username == data['teleusername']:
                                for cli in data['access_user']:
                                    if cli == msgsplit[2]:
                                        count = 1
                                        for user in datafetch():
                                            if user['username'] == cli:
                                                try:
                                                    bot.send_document(message.chat.id,open(f'data1/{user["userrole"]}/{msgsplit[1]}/{msgsplit[0]}/{msgsplit[2]}-{msgsplit[1]}.{msgsplit[0]}','rb'))
                                                except:
                                                    li = [filename.path.split(
                                                            '/')[-1] for filename in os.scandir(f'data1/{user["userrole"]}/')]
                                                    bot.send_message(message.chat.id,text=f"File for this date is not available\n\nAvalilable date is\n\nfrom this date {min(li)} To this date {max(li)}")
                                                    bot.send_document(message.chat.id,open(f'data1/{user["userrole"]}/{max(li)}/{msgsplit[0]}/{msgsplit[2]}-{max(li)}.{msgsplit[0]}','rb'))
                                                    bot.send_message(message.chat.id,text="Please Contact with support on telegram for assistance @rohanroy123")
                                if(count == 0):
                                    bot.send_message(message.chat.id,text=f"this client is not available in {data['username']}")
